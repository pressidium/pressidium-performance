...
class Feedback_API extends API {
    ...
    private function get_error_message( $response ): string {
        try {
            ...
            $error_message = $body['error'] ?? __( 'Unknown error', 'pressidium-performance' );
            ...
            return __( 'Unknown error', 'pressidium-performance' );
        }
    }
    ...
    public function send( string $reason, ?string $comment ): void {
        ...
        if ( $this->is_error( $response ) ) {
            ...
            throw new Feedback_Exception( esc_html( $error_message ) );
        }
